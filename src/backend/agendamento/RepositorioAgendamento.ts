...
const includeAgendamento =
  Prisma.validator<Prisma.AgendamentoInclude>()({
    planoAcao: {
      include: {
        pesquisa: {
          include: {
...
type AgendamentoComRelacionamentos =
  Prisma.AgendamentoGetPayload<{
    include:
      typeof includeAgendamento;
  }>;
...
function participantesDoJson(
  valor: Prisma.JsonValue
): ParticipanteAgendamento[] {
  return Array.isArray(
    valor
  )
    ? (valor as ParticipanteAgendamento[])
    : [];
}
...
function origemPlanoUsaPesquisa(
...
export default class RepositorioAgendamento {
  static async salvar(
    ...
    /*
     * O agendamento pode ser vinculado a qualquer tipo
     * válido de Plano de Ação:
     *
     * - Pesquisa de Clima
    ...
    if (
      ...
      if (
        origemPlanoUsaPesquisa(
          plano.tipoOrigem
        ) &&
        !plano.pesquisaId
      ) {
        throw new Error(
          "O plano selecionado não possui uma aplicação vinculada."
        );
      }
      ...
      if (
        ...
        throw new Error(
          "O plano selecionado não possui uma denúncia vinculada."
        );
      }
    }
    ...
    for (
      ...
      if (
        ...
        throw new Error(
          `Telefone inválido para ${participante.nome}.`
        );
      }
    }
    ...
    const dados: Prisma.AgendamentoUncheckedCreateInput =
      {
        ...
        local:
          agendamento.local?.trim() ||
          null,

        linkReuniao:
          agendamento.linkReuniao?.trim() ||
          null,
        ...
        participantes:
          participantes as unknown as Prisma.InputJsonValue,
      };
