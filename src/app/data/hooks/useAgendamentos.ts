...
export function useAgendamentos(
  ...
  const carregarAgendamentos = useCallback(async () => {
    try {
      ...
      setErro(error instanceof Error ? error.message : "Erro ao carregar agendamentos.");
    } finally {
      setCarregando(false);
    }
  }, [contexto]);
  ...
  const carregarAgendamentoPorId = useCallback(
    async (id: string) => {
      try {
        ...
        const mensagem =
          error instanceof Error ? error.message : "Erro ao carregar agendamento.";
        setErro(mensagem);
  ...
  const obterParticipantesCliente = useCallback(
    async (clienteId: string): Promise<ParticipanteClienteDisponivel[]> => {
      try {
        setErro(null);
        return await Backend.agendamentos.obterParticipantesCliente(clienteId);
      } catch (error) {
        const mensagem =
          error instanceof Error
            ? error.message
            : "Erro ao carregar participantes do cliente.";
        setErro(mensagem);
        throw error;
      }
    },
      ...
      new Promise<void>((resolve, reject) => {
        startTransition(async () => {
          try {
            ...
            setAgendamentoSelecionado((atual) => (atual?.id === id ? null : atual));
            resolve();
          } catch (error) {
            const mensagem =
  ...
  useEffect(() => {
    if (carregarAoIniciar) carregarAgendamentos();
