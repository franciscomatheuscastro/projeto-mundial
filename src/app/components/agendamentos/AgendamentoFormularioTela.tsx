...
export default function AgendamentoFormularioTela({
  ...
  const [
    ...
    useState<TipoAgendamento>(
      agendamentoInicial?.tipo ||
        "APRESENTACAO_PLANO"
    );
  ...
  const [
    ...
    >(
      agendamentoInicial
        ?.participantes
        ?.length
        ? agendamentoInicial.participantes.map(
            (
              participante
            ) => ({
              ...participante,
  ...
  /*
   * Os três módulos de avaliação usam plano.pesquisa.
   * Denúncia usa plano.denuncia.
   */
  ...
  useEffect(() => {
    ...
    async function carregar() {
      ...
      try {
        ...
        const dados =
          await obterParticipantesCliente(
            clienteSelecionado.id
          );
  ...
  async function salvar(
    ...
    if (
      ...
      setErroLocal(
        `Confira o telefone de ${
          telefoneInvalido.nome ||
          "um participante"
        }.`
      );
  ...
  function adicionarParticipanteVinculado() {
    ...
    if (
      ...
      setErroLocal(
        "Este participante já foi adicionado."
      );
            ...
            <div className="mt-5 grid gap-4 lg:grid-cols-2">
              ...
              <CampoSelect
                ...
                onChange={(
                  ...
                  setTipo(
                    valor as TipoAgendamento
                  )
                }
              >
                <option value="APRESENTACAO_PLANO">
            ...
            {clienteSelecionado && (
              <div className="mt-5 grid gap-3 rounded-2xl border border-blue-100 bg-blue-50/50 p-4 sm:grid-cols-[1fr_auto]">
                ...
                >
                  <option value="">
                    {carregandoParticipantes
                      ...
                      : "Selecione um participante do cliente"}
                  </option>


                  {participantesDisponiveis.map(
                    (
                      item
                    ) => (
                      <option
                        key={`${item.origem}:${item.id}`}
                        value={`${item.origem}:${item.id}`}
                      >
                        {
                          item.nome
                        }{" "}
                ...
                <button
                  type="button"
                  onClick={
                    adicionarParticipanteVinculado
                  }
                  disabled={
                    !participanteSelecionado
                  }
