    ...
    try {
      const resultado = await criarDenunciaPublica({
        ...
        nomeDenunciante: anonima
          ? null
          : String(
              formData.get("nomeDenunciante") || ""
            ).trim() || null,

        emailDenunciante: anonima
          ? null
          : String(
              formData.get("emailDenunciante") || ""
            ).trim() || null,

        telefoneDenunciante: anonima
          ? null
          : String(
              formData.get("telefoneDenunciante") || ""
            ).trim() || null,
      });
        ...
        try {
          ...
          setErroLocal(
            error instanceof Error
              ? `A denúncia foi registrada com o protocolo ${resultado.protocolo}, mas houve erro ao enviar um ou mais anexos: ${error.message}`
              : `A denúncia foi registrada com o protocolo ${resultado.protocolo}, mas houve erro ao enviar os anexos.`
          );
      ...
      <main className="flex min-h-screen items-center justify-center bg-slate-100 px-4 py-8">
        <div className="w-full max-w-lg rounded-3xl border border-slate-200 bg-white p-6 text-center shadow-sm sm:p-8">
          ...
          <p className="mt-2 text-sm leading-6 text-slate-500">
            Guarde este protocolo para acompanhar o
          ...
          <p className="mt-4 text-xs leading-5 text-slate-500">
            A consulta poderá ser feita informando este
            protocolo no canal de acompanhamento.
          </p>
      ...
      <section className="mx-auto max-w-4xl">
        <div className="mb-6 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
          ...
          <p className="mt-3 text-sm leading-6 text-slate-500">
            Registre sua denúncia de forma segura. Você
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <div className="md:col-span-2">
                ...
                <textarea
                  ...
                  className="w-full resize-y rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900 outline-none transition focus:border-blue-500 focus:ring-4 focus:ring-blue-100 disabled:cursor-not-allowed disabled:bg-slate-100"
                />
              </div>
  ...
  return (
    <div>
      ...
      <input
        ...
        required={required}
        disabled={disabled}
        className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-blue-500 focus:ring-4 focus:ring-blue-100 disabled:cursor-not-allowed disabled:bg-slate-100"
      />
    </div>
  );
}